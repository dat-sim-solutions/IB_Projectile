...
ax1.fill_between(data["t"], data["v_ana"] - data["v_t_theory_err"], data["v_ana"] + data["v_t_theory_err"], 
                 color='none', edgecolor='orange', hatch='//', alpha=0.7, label='Δv(Δm,ΔA), Setup Propagation')
ax1.plot(data["t"], data["v_ana"], 'k', label='Theory (Analytical)')
ax1.plot(data["t"], data["v_num"], 'g--', label='Numerical (Euler)')
ax1.scatter(data["t"], data["v_noisy"], color='red', s=8, alpha=0.7, label='Sensor Data')
...
Horizontal is the uncertainty in time frame, **Sync**ronization error.
The ball hitting a certain point and the camera actually recording that frame (Δt=0.05).
""")
